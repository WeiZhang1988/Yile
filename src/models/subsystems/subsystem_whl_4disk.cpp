...
void NMSPC::Subsys_Wheel_4Disk::pull_pv(const double &Gnd_Pz_fl, const double &Gnd_Pz_fr, const double &Gnd_Pz_rl, const double &Gnd_Pz_rr) {
    m_sptr_whl_fl->pull_pv(Gnd_Pz_fl);
    m_sptr_whl_fr->pull_pv(Gnd_Pz_fr);
...
const double &Axl_Trq_rr, const double &Brk_Prs_rr, const double &Tir_Fx_rr, const double &Tir_My_rr, const double &Tir_Fz_rr, const double &Sus_Fz_rr) {
    m_sptr_whl_fl->pull_fm(Axl_Trq_fl, Brk_Prs_fl, Tir_Fx_fl, Tir_My_fl, Tir_Fz_fl, Sus_Fz_fl);
    m_sptr_whl_fr->pull_fm(Axl_Trq_fr, Brk_Prs_fr, Tir_Fx_fr, Tir_My_fr, Tir_Fz_fr, Sus_Fz_fr);
    m_sptr_whl_rl->pull_fm(Axl_Trq_rl, Brk_Prs_rl, Tir_Fx_rl, Tir_My_rl, Tir_Fz_rl, Sus_Fz_rl);
    m_sptr_whl_rr->pull_fm(Axl_Trq_rr, Brk_Prs_rr, Tir_Fx_rr, Tir_My_rr, Tir_Fz_rr, Sus_Fz_rr);
}

void NMSPC::Subsys_Wheel_4Disk::push_fm(double &Brk_Trq_fl, double &Brk_Trq_fr, double &Brk_Trq_rl, double &Brk_Trq_rr) {
    m_sptr_whl_fl->push_fm(Brk_Trq_fl);
    m_sptr_whl_fr->push_fm(Brk_Trq_fr);
    m_sptr_whl_rl->push_fm(Brk_Trq_rl);
    m_sptr_whl_rr->push_fm(Brk_Trq_rr);
}

void NMSPC::Subsys_Wheel_4Disk::push_drv(d_vec &derivatives) {
    m_sptr_whl_fl->push_drv(m_whl_drvs_fl);
    m_sptr_whl_fr->push_drv(m_whl_drvs_fr);
    m_sptr_whl_rl->push_drv(m_whl_drvs_rl);
    m_sptr_whl_rr->push_drv(m_whl_drvs_rr);
