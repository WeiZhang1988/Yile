...
void run_Pass14DOF(string par_file,string output_file) {
	std::ofstream of;
	Simulator_Pass14DOF sim1 = Simulator_Pass14DOF(par_file,0.0,100.0,5e-4);
	steady_clock::time_point start = steady_clock::now();
	sim1.run();
	steady_clock::time_point end = steady_clock::now();
	milliseconds dur= duration_cast<milliseconds>(end - start);
...
int main(int argc, char *argv[]){
	string par_file = "data/parameters/parameters.json";
	string output_file = "data/outputs/res_pass14dof.csv";
	if (argc>2) {
		par_file = argv[1];
		output_file = argv[2];
	} else {
